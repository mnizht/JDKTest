...
public class MoneyUtils {
  ...
  public static String digitUppercase(long money) {
    ...
    return stringUppercase(String.valueOf(money));
  }

  private static String stringUppercase(String strMoney){
    // numIndex用于选择金额数值
    int numIndex = 0;
    // unitIndex用于选择金额单位
    ...
    for (; numIndex < strMoney.length(); numIndex++, unitIndex++) {
      ...
      if (num == '0') {
        isZero = true;
        // 如果当前位是亿、万、元，且数值为零
        if (isBillionOrTenThousandOrYuan(unitIndex)) {
          //补单位亿、万、元
  ...
  private static boolean isBillionOrTenThousandOrYuan(int unitIndex){
    return UNIT[4].equals(UNIT[unitIndex]) || UNIT[0].equals(UNIT[unitIndex])
